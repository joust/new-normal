...
export function Uno (locale, content, host) {
  ...
  function resolveDiscuss (hands, decks, card) {
    ...
    hands.forEach((hand, index) => {
      ...
      if (idiot && firstI) { hand.push(`${firstI}*`) } else if (!idiot && firstS) { hand.push(`${firstS}*`) }
  ...
  function resolveResearch (hand, decks, top, idiot) {
    ...
    if (isAppealTo(top)) {
      ...
      if (cards.length < nrOfCards) { cards = [...cards, ...deck.filter(isArgument).slice(0, nrOfCards - cards.length)] }
    } else {
      const topic = topicOf(top)
      cards = deck.filter(c => isArgument(c) && hasTopic(c, topic)).slice(0, nrOfCards)
      if (cards.length < nrOfCards) { cards = [...cards, ...deck.filter(isAppealTo).slice(0, nrOfCards - cards.length)] }
    }
  ...
  return {
    ...
    moves: {
      playCard: ({ G, ctx, events }, index, alt) => { // alt = alternative card to put on the pile
        ...
        switch (cardType(card)) {
          ...
          default: // argument
            ...
            if (isWildcard(card)) { resolveDiscuss(G.hands, G.decks, card) } else { hand.splice(index, 1) }
            G.pile.push(alt)
        }
      },
      drawCard: ({ G, ctx }) => {
      ...
      setName: ({ G, ctx }, id, name) => {
        if (ctx.currentPlayer === G.host) { G.names = G.names.map((n, i) => i == id ? name : n) } else { return INVALID_MOVE }
      }
    ...
    endIf: ({ G, ctx }) => {
      ...
      const hand = G.hands[ctx.currentPlayer]
      const top = G.pile.length ? G.pile[G.pile.length - 1] : undefined
      if (!hand.length && (isArgument(top) || isAppealTo(top) || isDiscuss(top)) && isOfType(top, idiot)) { return { winner: ctx.currentPlayer } } // TODO >2 multiplayer ends, when all of one side are finished
    },
    minPlayers: 2,
